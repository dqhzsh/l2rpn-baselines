...
from DQN.prioritized_replay_buffer import RankPrioritizedReplayBuffer
from DQN.dqn_NN import DeepQ
...
class DeepQAgent(AgentWithConverter):
    ...
    def GGF_score3(self,num):
        ...
        q_sa = tf.reduce_sum((Q * selected_actions),axis=1)

        temp = tf.expand_dims(tf.Variable(r_batch, dtype=tf.float32), axis=0)
        r_GGF_score = self.deep_q.GGF(temp[:, :, :self._training_param.n_user], self._training_param.n_user, 2)
        GGF_score = (r_GGF_score * q_sa) / np.dot(np.array(r_batch), self._training_param.weight_coef)
        Reward = q_sa
        ...
        with tqdm(total=iterations - step, disable=False, miniters = 500) as pbar:
            while step < iterations:
                ...
                if step > num_pre_training_steps:                    #预训练仅探索不训练网络
                    ...
                    combined_rewards = np.dot(np.array(r_batch), self._training_param.weight_coef)
                    loss, loss_AE, priorities = self.deep_q.train(s_batch, a_batch, combined_rewards, d_batch, s2_batch, w_batch, idx_batch,
                                             self._training_param.MINIBATCH_SIZE)
                ...
                if done:                   #每次epoch结束输出结果
                    ...
                    #     tf.summary.scalar('Reward', epoch_Reward, step=epoch)
            ...
            # 画每个epoch的reward图和存活步数图,GGFscore图

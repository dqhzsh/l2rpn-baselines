...
class DeepQ(object):
    ...
    def construct_q_network(self):
        ...
        q_output = Dense(self.action_size, name="layer_Qnet_output")(lay_Qnet)
        self.model_AE_head   = Model(inputs=[input_layer], outputs=[reverse_observation],name="AE-{}".format(self.training_param.model_name))
        self.model_Qnet_head = Model(inputs=[input_layer], outputs=[q_output],name="Qnet-{}".format(self.training_param.model_name))
        self.model           = Model(inputs=[input_layer], outputs=[q_output, reverse_observation], name=self.training_param.model_name)
        # self.model.compile(loss='mse', optimizer=Adam(lr=self.lr_,clipnorm = 0.5))
        print(self.model.summary())
        self.target_model = Model(inputs=[input_layer], outputs=[q_output])
    ...
    def train(self, s_batch, a_batch, r_batch, d_batch, s2_batch, w_batch, idx_batch, batch_size):  # priority buffer的需多加输入idx_batch, batch_size
        ...
        target_q = done * q_target_value                                                     # 非done的target_q_value
        max_future_q = np.max(target_q,axis=1)                                               # 不需要求a*，直接求Q^(s'，a*)
        q_sa_selected_target = rew_batch + self.training_param.gama * max_future_q           # 求出更新后的Q(s,a)，=r+γQ^(s'，a*),包括done的
        ...
        self.model.compile(optimizer=Adam(learning_rate=self.lr_, clipnorm=0.5),
                           ...
                           run_eagerly=True)
        h = self.model.fit(x=s_batch, y={'layer_Qnet_output': y_batch,
                                         'layer_AE_head_output': s_batch},
        ...
        batch_sq_error = tf.reduce_mean(sq_error)
        batch_td_error = q_sa_selected - q_sa_selected_target
        # batch_td_error = tf.reduce_mean(td_error)
    ...
    def predict_movement(self, data, epsilon):
        ...
        return opt_policy, q_values[0: opt_policy], q_values

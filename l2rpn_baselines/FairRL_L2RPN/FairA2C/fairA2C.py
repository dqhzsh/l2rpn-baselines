...
from FairA2C.fairA2C_NN import A2C
...
class Agent(AgentWithConverter):
    ...
    def init_deep_q(self, transformed_observation):
        if self.deep_q is None:
            ...
            print(transformed_observation.shape[-1])
    ...
    def load(self, env, path):
        ...
        self.deep_q = A2C(self.action_space.size(), observation_size=self.observation_size, lr=self.lr,
    ...
    def train(self,
        ...
        # self.observation_size =env.observation_space.size_obs()
        print("状态空间大小：{}".format(self.observation_size))
        ...
        with tqdm(total=epochs*iterations, disable=False, miniters=1, mininterval=3) as pbar:
            for epoch in range(epochs):
                ...
                # 回合更新
                loss_actor, loss_critic, loss_AE = self.deep_q.train(np.array(states), np.array(actions), np.array(rewards),

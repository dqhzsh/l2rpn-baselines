...
class A2C(object):
    ...
    def __init__(self, action_size, observation_size, lr=1e-4,
    ...
    def custom_loss(self):
        def loss(data, y_pred):
            ...
            weight_coef2 = tf.constant(self.training_param.weight_coef, dtype=tf.float32)
            loss_actor2 = tf.tensordot(p2, weight_coef2, axes=1)  # 不带GGF
            ...
            loss_GGF = self.GGF(p2[:, :self.n_user], self.n_user, 2)  # loss with GGF
            loss_GGF = tf.reduce_mean(loss_GGF)
            loss_actor -= self.training_param.alpha * loss_GGF
    ...
    #     self.model             = Model(inputs=[input_layer], outputs=[soft_proba, v_output],name=self.training_param.model_name)
    ...
    def construct_network(self):
        ...
        #encoder level1
        # lay_encoder = Dense(512,name="layer_encoder_hidden1")(input_layer)
        # lay_encoder = Activation("relu")(lay_encoder)
        # lay_encoder = Dropout(0.05)(lay_encoder)

        #encoder level2
        lay_encoder = Dense(128,name="layer_encoder_hidden2")(input_layer)
        lay_encoder = Activation("relu")(lay_encoder)
        lay_encoder = Dropout(0.05)(lay_encoder)
        ...
        # decoder level2
        # lay_decoder = Dense(512, name="layer_decoder_hidden2")(lay_decoder)
        # lay_decoder = Activation("relu")(lay_decoder)
        # lay_decoder = Dropout(0.05)(lay_decoder)
    ...
    def predict_movement(self, data, epsilon):
        ...
        data = data.reshape(1, self.observation_size)
    ...
    def train(self, s_batch, a_batch, r_batch, s2_batch, d_batch, pa_batch):
        ...
        # TODO 添加AE的训练
        # h = self.model.fit(x=s_batch, y={'layer_actor_head_output': np.append(act_batch, adv_batch, axis=1),
        #                                  'layer_critic_head_output': V_target},
        #                    batch_size=32, epochs=3, shuffle=true, callbacks=[cb])
        reverse_obs_batch = s_batch
        h = self.model.fit(x=s_batch, y={'layer_actor_head_output':np.append(act_batch,adv_batch,axis = 1), 'layer_critic_head_output':V_target, 'layer_AE_head_output':reverse_obs_batch},
    ...
    #     path_model, path_policy_model, path_critic_model = self._get_path_model(path, name)
    #     self.model = load_model('{}'.format(path_model))
    #     self.model_policy_head = load_model('{}'.format(path_policy_model))
    #     self.model_critic_head = load_model('{}'.format(path_critic_model))
    #     print("Successfully loaded network from: {}".format(path))
